...
int fd_log;         //writes to the logfile
char *location404;  //error_404 html file
size_t filesize404; //size of the error 404 file
char *serverRoot;
char *serverName;
...
{ 
    ...
    fd_log = ((HTTP_REQUEST *)http_req)->fd_log;

    location404 = ((HTTP_REQUEST *)http_req)->file404;
    filesize404 = ((HTTP_REQUEST *)http_req)->filesize404;
    serverRoot = ((HTTP_REQUEST *)http_req)->serverRoot;
    serverName =  ((HTTP_REQUEST *)http_req)->serverName;
    ...
    if (client_socket == -1)
    {
        ...
        { /* abnormal error */
            perror("accept");
            write_to_logfile(fd_log, "error accepting, reason: client_socket == -1");
        }
        return NULL;
    }
    ...
    char *log_msg = malloc(sizeof(char) * 100);
    strcpy(log_msg, "request received with: ");
    // strcat(log_msg,bytes);
    char* bytes_string = getString(bytes);
    strcat(log_msg,bytes_string);
    free(bytes_string);

    strcat(log_msg, "bytes\n");
    write_to_logfile(fd_log, log_msg);
    free(log_msg);
    int ret = header_parse(buff, bytes, http_req);
    printf("header_parse: ret = %d",ret);
    ...
    while (check < (buff + len))
    {
        ...
        if (status == ONE_LF)
        {
            *line_end = '\0';
            ...
            if (http_req->PARSE_HEAD_OPTION)
            { //parse header options, in key:value pairs
                if (process_option_line(header_line) == -1)
                {
                    perror("process_option_line error");
                    write_to_logfile(fd_log, "process_option_line error");
                    return -1;
                }
            }
            else
            { //parse the header
                if (process_logline(buff, http_req) == -1)
                {
                    ...
                    write_to_logfile(fd_log, "process_logline error");
                    return -1;
                }
            }
        ...
        // #ifdef DEBUG
            printf("process_header_end:retval %d",retval);
        // #endif
...
int process_option_line(char *buff)
{
    ...
    *value++ = '\0'; /* overwrite the : */
    ...
    else if (!memcmp(line, "CONTENT_TYPE", 13))
    {
        ...
        char *log_msg = malloc(sizeof(char)*100);
        strcpy(log_msg, "Request Type: ");
        strcat(log_msg, content_type);
        write_to_logfile(fd_log, log_msg);
    }
...
int process_logline(char *buff, HTTP_REQUEST *http_req)
{
    ...
    char *log_msg = malloc(sizeof(char) * 50);
    strcpy(log_msg, "Request Type: ");
    strcat(log_msg, method);
    write_to_logfile(fd_log, log_msg);
    free(log_msg);
...
int process_header_end(HTTP_REQUEST *http_req)
{
    ...
    {
        serve_static(*(http_req->pclient), location404, filesize404); //line:netp:doit:beginnotfound
    ...
    { /* Serve static content */
        ...
        { //line:netp:doit:readable
            serve_static(*(http_req->pclient), location404, filesize404);
...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
    ...
    { /* Static content */   //line:netp:parseuri:isstatic
        ...
        memcpy(filepath,uri+1,strlen(uri));
        strcpy(filename,filepath);
        prepend(filename, serverRoot);
        free(filepath);     

        // memcpy()
        printf("filepath is %s\n", filename);
        //If the requested uri equals to '/'
        //sends the default index.
...
void serve_static(int client_socket, char *filename, int filesize)
{
    ...
    int srcfd;
    char *srcp, filetype[MAXLINE/2], buf[MAXLINE];

    //Send response headers to client
    char* buffer_sprintf = buf;

    get_filetype(filename, filetype);    //line:netp:servestatic:getfiletype
    sprintf(buf, "HTTP/1.0 200 OK\r\n"); //line:netp:servestatic:beginserve
    sprintf(buf, "%sServer: %s \r\n", buffer_sprintf, serverName);
    sprintf(buf, "%sContent-length: %d\r\n", buffer_sprintf, filesize);
    
    sprintf(buf, "%sContent-type: %s\r\n\r\n", buffer_sprintf, filetype);

    if (response_write(client_socket, buf, strlen(buf)) != strlen(buf)) //line:netp:servestatic:endserve
    {
        if(client_socket<0)
            linux_error("response_write");
    }
    ...
    if (response_write(client_socket, srcp, filesize) != (size_t)filesize)  //line:netp:servestatic:write
        linux_error("response_write");
...
size_t response_write(int client_socket, void *usrbuf, size_t n)
{
    size_t nleft = n;
    ssize_t nwritten;
    char *bufp = usrbuf;
...
void serve_dynamic(int client_socket, char *filename, char *cgiargs)
{
    ...
    if (response_write(client_socket, buf, strlen(buf)) != strlen(buf))
        linux_error("response_write");
